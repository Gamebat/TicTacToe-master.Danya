...
public class Game {
    ...
    public void move(int x, int y) throws UserException {
        if (x < 0 || x >= size)
            throw new UserException("x за пределами поля" + System.lineSeparator() + "====================");
        if (y < 0 || y >= size)
            throw new UserException("y за пределами поля" + System.lineSeparator() + "====================");
        if (field[x][y].getCell() != Cell.EMPTY)
            throw new UserException("Ячейка занята x = " + (x+1) + " y = " + (y+1) + System.lineSeparator() + "====================") ;
